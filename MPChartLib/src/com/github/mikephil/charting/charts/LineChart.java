        ...
        if (mDrawYValues && mData.getYValCount() < mMaxVisibleCount * mTrans.getScaleX()) {
            ...
            for (int i = 0; i < mData.getDataSetCount(); i++) {
                ...
                for (int j = 0; j < positions.length * mPhaseX; j += 2) {
                    ...
                    int realOffset = valOffset;
                    if(dataSet.isBottomValues()) {
                    	for(int c=0; c<mData.getDataSetCount(); c++) {
                    		LineDataSet concurrentDataSet = dataSets.get(c);
                    		if(concurrentDataSet == dataSet) {
                    			continue;
                    		}
                    		float concurrentVal = concurrentDataSet.getYVals().get(j / 2).getVal(); 
                    		if(concurrentVal>0) {
                    			realOffset = (int)(- realOffset * 2f);
                    		}
                    	}
